...
public:
  ...
  BSTNode<Data>* successor() {
    BSTNode<Data>* cur;
    cur = this;
    if (this -> right != nullptr){             //Case1: The current node has right child.
      cur = this -> right;
      while(cur-> left != nullptr)             //Find the leftmost node of its right subtree.
	cur = cur -> left;
      return cur;
    }	
    else if (cur -> parent != nullptr){        //Case2: The current node doesn't have right child.
      while (cur -> parent -> left != cur ){   //Find the parent of current node of which left child is current node.
	cur = cur -> parent;
	if(cur -> parent == nullptr){         //If the parent dosen't exist,then return null.
	  return nullptr;	
	}
      } 
      cur = cur -> parent;                    //if current node is the left child of its parent,then return its parent.
      return cur;
    }
    else cur = nullptr;                       
    return cur;	
